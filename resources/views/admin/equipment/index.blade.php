    ...
    <main class="nxl-container">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="proposalSent">
            <div class="offcanvas-header ht-80 px-4 border-bottom border-gray-5">
                <div>
                    <h2 class="fs-16 fw-bold text-truncate-1-line">Create Equipment</h2>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.equipment.store') }}" method="POST" id="roleForm">
                @csrf
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="proposalSent_edit">
            <div class="offcanvas-header ht-80 px-4 border-bottom border-gray-5">
                <div>
                    <h2 class="fs-16 fw-bold text-truncate-1-line">Edit Equipment</h2>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            <div class="load_edit">
        ...
        <script>
            ...
            $(document).on('click', '.edit_model', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('admin.equipment.edit') }}",
                    data: {
                        id: id
                    },
                    cache: false,
            ...
            $(document).on('click', '.delete_model', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value == true) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "{{ route('admin.equipment.delete') }}",
                            data: {
                                id: id,
                                 _token: "{{ csrf_token() }}"
                            },
                            ...
                            success: function(data) {
                                console.log('response', data);
                                $(function() {
                                    toastr.success('Equipment Deleted Success', {
                                        timeOut: 1000,
                                        fadeOut: 1000,
                                    });
                                });
